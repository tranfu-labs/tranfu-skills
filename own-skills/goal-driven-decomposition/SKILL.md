...
description: Solve "build/make/design X" goals AND follow-up edits on such goals (e.g. "做一个网站", "build a tool", "加用户登录", "X 偏了改一下", "改一下 UI"). All state lives in goal-docs/ as @mention'd .md files. Cold-start: (1) prior-weight modal shape+intent+user; (2) write 00-用户画像-r1.md as roleplay-able persona (审美参考 split into hero-refs / anti-refs; each ref includes a local screenshot path under goal-docs/_refs/, not just product names); (2.5) label each dim's **track** — **enumerate** (功能/技术/部署/数据模型/调度: text-decomposable spec) vs **sample** (UI/视觉/排版/文案 tone/品牌调性: continuous holistic spec, NOT losslessly text-encodable); (3) for each stable dim (website→功能/UI/技术/部署) run track-appropriate doc-relay — **Step 2-E** (enumerate: persona-brief Agent (sync) → design-draft Agent (sync, text-only) → persona-reaction Agent (MANDATORY, async, sonnet, run_in_background=true) → design-final only on pushback) OR **Step 2-S** (sample: design-draft Agent outputs 1-3 candidate PNGs from HTML+CSS single-file render BEFORE writing design.md; persona-reaction MUST view PNGs before signing; design-final must re-render a PNG, text-only patch forbidden); (4) build artifact; (5) MANDATORY validation-fix loop: plan Agent writes 90-validation-plan-r{N}.md (every check has severity=critical|nice + trace; **sample-track dims MUST include ≥1 critical blind-compare check** — independent sonnet sees {our render + N hero-refs + N anti-refs from persona's 审美参考} and picks "most-like-anti-ref"; ours-not-picked = pass; atomic property checks on sample-track dims demoted to anti-trope guardrails — necessary, not sufficient), validation Agent → fix Agent ≤3 iter using any tools (chrome-devtools mcp, bash, lighthouse). Iteration mode (existing goal-docs/): Step 0 detects, scope Agent classifies as 新增/修正/画像变更 AND records track per affected dim (track-change between rounds, enumerate↔sample, = major redesign requiring sample redraw from scratch) and writes UPDATE-r{N}-scope.md; 新增 → re-run Step 2 only on affected dims; 修正 → skip Step 2, add new check with trace=user's verbatim words; 画像变更 → surface to user (do NOT auto-rewrite persona). Filename: {模块号}-{模块名称}-{用户级轮次}-{对话/iter轮次}-{角色}.md (dims) or {模块号}-{模块名称}-{用户级轮次}-[{iter}-]{角色}.md (validation/persona). Iter validation = delta plan (Inherited/New/Overrides/Deprecated) + resolved cache. Hard NOs: inline answers, SendMessage, skip reaction Agent, self-score the artifact, claim "inherited still pass" without re-running, cold-start protocol on existing goal-docs/, skip protocol on user-facing goals like "做一个蒙卡模拟网站", **running enumerate Step 2-E protocol on a sample-track dim, persona-reaction signing without viewing rendered PNGs, declaring sample-track dim done on atomic-check pass alone without blind-compare, persona's 审美参考 storing only product names without local screenshot paths**. Surface to user after 3 failed validation iterations. Only skip whole protocol for pure one-shots with zero design dimensions. **(8th iter / Appendix H — see body)**: adds true_goal under USER GATE at intake, goal-tree thin-slice loop, role-anchored build-plan with `§implicit-decisions`, per-slice milestone gate + real-artifact acceptance walkthrough, project-end retro feeding anti-refs back to persona. New hard NOs: silent goal reinterpretation, >1 layer recursion, per-dim parallel without slice gates, build-plan without `§implicit-decisions`. Do NOT trigger when: (a) 纯一次性问答 / 零设计维度的指令 (e.g. "grep 一下 X" / "改这行 typo"); (b) 已有 goal-docs/ 但用户只是普通问问题, 没说 "改 / 加 / 偏了"; (c) 走另一个明确 skill 的范围 (e.g. 商业可行性评估 → business-analysis-pipeline; 审稿 → credibility-review; 发布 skill → tranfu-publish).
version: 0.1.0
