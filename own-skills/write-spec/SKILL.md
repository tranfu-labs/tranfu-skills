...
### 公司库内
- [openspec](../external-skills/openspec/SKILL.md) — 多轮迭代项目用 propose→apply→archive 管理变更规范; **本 skill 区别**: write-spec 从零生成单次 PRD，openspec 管理已有项目的持续规范演化

### 外部世界
- [prd (snarktank)](https://bestclaudecodeskills.com/skills/snarktank-ralph-prd-claude-skills) — 通过问答生成 PRD; **本 skill 区别**: write-spec 含完整 MoSCoW 分级框架和 acceptance criteria 模板，输出更结构化
- [prd-generator (jamesrochabrun)](https://github.com/jamesrochabrun/skills/blob/main/skills/prd-generator/SKILL.md) — PM 导向的 PRD 生成; **本 skill 区别**: write-spec 内置 scope creep 防护和 non-goals 强制段，面向工程+产品双角色

### 本 skill 独特价值
- 输出含 P0/P1/P2 分级 + Given/When/Then 验收标准
- 内置 non-goals 段防止 scope creep
- 支持从模糊一句话到完整 PRD 的渐进引导
