...
1. **MUST 通过 `PATCH /api/v1/services/{service-uuid}` 修改 sub-application 任何字段**; **NEVER 直接 `PATCH /api/v1/applications/{sub-uuid}`** (必 404)——sub-application 不在 `/applications` 命名空间。
2. **`sub-application.fqdn` 是只读派生字段**: **NEVER 直接 PATCH 它**; **MUST 通过 `PATCH /api/v1/services/{service-uuid}` 的 `urls` 字段修改**, Coolify 内部同步到对应 sub-application 的 fqdn。详见 [urls-vs-docker-compose-domains.md](urls-vs-docker-compose-domains.md)。
3. **`service_type` 字段对 Docker Compose Empty 是 `null`**——它是 one-click services（如 `postgresql` / `actualbudget`）才有的字段。**MUST NEVER 在 POST 创建 Docker Compose Empty 资源时传 `type` 字段**; 传了 Coolify 会按 one-click 模板查询, 返回 404/422。
