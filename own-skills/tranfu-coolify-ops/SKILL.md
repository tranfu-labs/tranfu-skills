...
错误命名或错误归宿下创建资源（创建容易、回滚要 delete + project delete，代价不小）。
...
**操作前先确认 context**。多实例环境里默认 context 容易选错；每次进入场景脚本前先跑
`coolify context list`，向用户报"当前在哪个实例"。需要临时切实例时用 `--context=<name>` 覆盖单条命令，
不动 default context。
...
Step 0：跑 `coolify context list`，确认在生产实例。跑 `coolify server list --format json`，
长度 == 1，拿到 `server-uuid = abc-123`。

Step 1：解析 URL → org=tranfu-labs，repo=foo-app；正则匹配通过。

Step 2：`coolify app list --format json | jq -e '.[] | select(.name == "foo-app")'` 无命中。

Step 3：project 不存在 → `coolify project create --name foo-app`，拿到 `project-uuid = def-456`。

Step 4：`coolify github list --format json` 长度 == 1，拿到 `github-app-uuid = ghi-789`。

Step 5：`coolify github repos ghi-789 --format json | jq -e '.[] | select(.full_name == "tranfu-labs/foo-app")'` 命中。

Step 6：跑 `coolify app create github --name foo-app --project-uuid def-456 --server-uuid abc-123
--github-app-uuid ghi-789 --git-repository tranfu-labs/foo-app --git-branch main
--build-pack dockercompose --ports-exposes 80 --instant-deploy`。

Step 7：拿到新 app UUID，跑 `coolify app deployments list <new-uuid>`，再
`coolify app deployments logs <new-uuid> <dep-uuid> --follow` 跟踪首次部署。
</example>
...
- 没校验命名，给 `Foo_App` 这种不合规仓库照样建出来，事后清理要 `app delete` + `project delete`。

正确做法：每个场景都先跑 `commands/prerequisites.md` 的前置，再走 `commands/tranfu-naming.md` 的命名校验，
再进入场景脚本主体。一旦任何断言失败，按场景的终止文案返回，绝不静默修复或绕过。
</bad-example>
