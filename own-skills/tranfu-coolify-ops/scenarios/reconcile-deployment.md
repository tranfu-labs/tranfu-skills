...
跑 reconcile 前 create 9 项 TODO，每跑完一个 Step 更新状态：

```
[ ] Step 0: 环境前置
[ ] Step 1: Token & Base URL 活性
[ ] Step 2: 仓库代码侧四件套
...
## Step 0: 环境前置（read-only）

跑 [../commands/prerequisites.md](../commands/prerequisites.md) 的 5 个 check：

- `gh` CLI 装且 auth
- `jq` / `curl` 装
- 在 git 仓库根目录
- `$COOLIFY_API_TOKEN` 存在（不验活）
- 仓库命名符合 tranfu 约束（org=tranfu-labs, name=烤肉串-app）
...
```bash
bash .claude/worktrees/beautiful-pasteur-d0fd5e/own-skills/tranfu-coolify-ops/assets/check-token.sh "$BASE"
```

或本地拷贝路径：`assets/check-token.sh`。

- 输出 `✓` → check 通过，next step
- 输出 `✗ <reason>` → 终止，原文显示给用户

**Diff / Act**：无 act——这步只读 + fail-fast。

**Act 失败处理**：不会有 act，只会有 check fail；按 check-token.sh 返回的原因找 token 问题。
