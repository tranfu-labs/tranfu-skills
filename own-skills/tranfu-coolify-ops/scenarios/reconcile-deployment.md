...
- **作用域硬约束**：只处理当前 GitHub repo 对应的那一个 project / application，**不扫描、不列举、不操作 Coolify 实例上其他资源**
- **资源类型唯一**：0.8 只识别 / 操作 Application (private-github-app + dockercompose) namespace, 不读不写 `/services`。同名 project 下若残留旧 Service 形态资源 (0.7 历史包袱), skill 不接管不迁移
...
- **同名硬约束**：`REPO_NAME == PROJECT_NAME == APP_NAME`，从 GitHub URL 唯一派生

## 工作单元契约

- **输入**：
  - 用户触发语，**必须含一个 GitHub URL**（如 `https://github.com/tranfu-labs/markdown-kits-app`）
  - URL 推导唯一锚点：`REPO_NAME = PROJECT_NAME = APP_NAME = <x>-app`
  - cwd 不要求事先在 repo——Step 2I / 更新分支 D 决定要不要 mktemp clone
...
```
Step 0: preflight (工具 / 凭据 / Coolify 活性 / GitHub App integration)
   ↓
Step 1: 同名 project + Application 探测
   ↓
┌──── project 不存在 (或空壳) ──┐   ┌──── Application 已存在 ─────────────┐
│ 初始化部署分支 (固定流程)      │   │ 更新部署分支 (按意图条件触发)        │
│ Step 2I:  mktemp + clone       │   │ Step 2U: 按用户意图 act (可组合):    │
│ Step 3I:  compose 合规         │   │   A. redeploy → POST /deploy         │
│           (subagent 修)        │   │   B. 改域名 → PATCH                  │
│ Step 4I:  POST project +       │   │      docker_compose_domains          │
│           Application          │   │   C. 改 env → POST envs + A          │
│ Step 5I:  GH secrets +         │   │   D. 改源码/compose →                │
│           auto env + vars      │   │      mktemp clone + subagent +       │
│ Step 6I:  application env      │   │      push (compose 走 git 不 PATCH)  │
│ Step 7I:  autonomous push      │   │   E. 改 GH 配置 → gh secret/var set  │
│                                │   │ (agent autonomous, 不依赖 user cwd)  │
└─────────────┬──────────────────┘   └─────────────┬────────────────────────┘
              ↓                                ↓
              └─────── 共用收尾 (按条件 skip / act) ──────┘
...
**特例分流: 同名 project 下只有 Service 没有 Application** (0.7 历史项目):
- skill 0.8 **不接管**, 告知用户 "已存在旧 Service 形态资源 (uuid=...), 0.8 仅管 Application 形态。如要切换请手动 DELETE 旧 service 后重跑 (走 reversible-ops 确认流, 注意保留 volume)", 终止。
- skill 0.8 不主动 GET `/services`, 只有当 Application 不存在且 project 已存在时, 才顺手 GET 一次 `/services` 以便给出明确提示 (而不是默默走初始化分支创建 Application, 撞 409)。
...
```
[ ] Step 0:  preflight (含 GitHub App integration 校验)
[ ] Step 1:  入口分流 → 初始化
[ ] Step 2I: mktemp 临时目录 + git clone
[ ] Step 3I: compose 合规 (不合规 spawn subagent)
[ ] Step 4I: 建 project + Application, 告知用户 $APP_UUID
[ ] Step 5I: GH secrets + 自动建 environment + vars
[ ] Step 6I: application env (POST /envs)
...
**更新分支 TODO**（Application 已存在, agent autonomous, 不依赖 user cwd）— 按用户意图条件创建：
...
- **Coolify**：`$COOLIFY_API_TOKEN` 存在 + `/api/v1/version` 200 + 写权限探测 (PATCH dummy application uuid)
- **GitHub App integration**：`GET /api/v1/github-apps` 找 organization=tranfu-labs 的一条, 拿 `$GITHUB_APP_UUID` 输出。不存在 → ✗ + Coolify UI 装 GitHub App 的入口
...
|---|---|---|
| `0` | 全 ✓ | 进 Step 1 |
| `1` | 硬 check ✗（工具 / 凭据 / 命名 / GitHub App 不存在）| 终止 |
...
$GITHUB_APP_UUID  = <Coolify GitHub App integration uuid, organization=tranfu-labs>
...
## Step 1: 同名 project + Application 探测（入口分流）
...
if [ -z "$PROJECT_UUID" ]; then
  ...
  echo "✓ Project '$PROJECT_NAME' 已存在: $PROJECT_UUID"

  # 1.2 在该 project 下找同名 Application (双重 filter 防跨 project 误命中)
  ...
  if [ -n "$APP_UUID" ]; then
    echo "✓ Application '$APP_NAME' 已存在: $APP_UUID → 走更新部署分支"
    # 直接进 Step 2U
  else
    # 1.3 Application 不存在; 看是否有旧 Service 形态资源残留 (0.7 历史包袱)
    LEGACY_SVC=$(curl -sS -H "Authorization: Bearer $COOLIFY_API_TOKEN" \
      "$BASE/api/v1/services" \
      | jq -r --arg n "$APP_NAME" --arg p "$PROJECT_UUID" \
        '.[] | select(.name == $n and .project_uuid == $p) | .uuid')

    if [ -n "$LEGACY_SVC" ]; then
      cat <<EOF
✗ Project '$PROJECT_NAME' 下已存在旧 Service 形态资源 ($LEGACY_SVC), 但没有 Application 形态资源.
...
> Coolify API 没暴露 `?name=` filter，list 一次 + jq filter 是唯一方式；但**只接受 name 完全等于锚点那一条**，不展示其他资源、不让用户选。

**分流**：

- Application 存在 → 跳到 [Step 2U](#更新部署分支step-2uproject-已存在)（更新部署分支）
- Application 不存在 + project 空壳或不存在 → 进 [Step 2I](#step-2i-git-clone-到临时目录)（初始化部署分支）
- Application 不存在但同名 Service 残留 → 终止 (上面 case)
...
**Check**（关键约束：`image` 必须有、`build` 必须没有——Coolify 不构建镜像, 走 GHCR pull）：
...
if [ -f compose.yml ]; then
  grep -qE '^\s*build\s*:' compose.yml && COMPOSE_ISSUES+=("compose.yml 含 build: (Coolify 不构建镜像, 主 service 只能 image: ghcr.io/...)")
...
- `COMPOSE_ISSUES` 空 → ✓ skip，进 Step 4I
- 非空 → 进 Act
...
Agent({
  ...
  prompt: "你的任务：在 cwd ($WORKDIR/$REPO_NAME) 内, 按照 SKILL_ROOT/own-skills/tranfu-coolify-ops/references/file-generation-rules.md 的规则, 把仓库改造成 Coolify (Application + dockercompose build pack) 可部署形态。

  必须满足的硬约束:
  - compose.yml 主 service 只写 image: ghcr.io/<org>/<repo>:<tag>, 禁止 build: (Coolify dockercompose build pack 看到 build: 会尝试自己 build, 我们要走 GHCR pull)
...
### Step 4I: 创建 Coolify project + Application

**前提**：Step 1 已确认 Application 不存在（project 可能存在但空壳, 也可能完全不存在）；Step 3I 已让源码合规。

**Act**：

```bash
# 4I.1 建同名 project (仅 project 不存在时跑; 空壳时跳过, 复用 Step 1 拿到的 $PROJECT_UUID)
if [ -z "$PROJECT_UUID" ]; then
...
# 4I.2 解析 compose 拿主 sub-application 名 + 端口 (后面 docker_compose_domains / ports_exposes 要用)
MAIN_SVC=$(yq '.services | keys | .[0]' compose.yml 2>/dev/null || \
  awk '/^services:/{f=1;next} f && /^[[:space:]]+[a-z0-9_-]+:/{gsub(/[[:space:]:]/, ""); print; exit}' compose.yml)
MAIN_PORT=$(yq ".services.${MAIN_SVC}.expose[0]" compose.yml 2>/dev/null || \
  awk -v svc="$MAIN_SVC" '$0 ~ "  "svc":"{f=1} f && /expose:/{g=1;next} g && /- /{gsub(/[^0-9]/, ""); print; exit}' compose.yml)

# 4I.3 建 Application (一次带 git binding + dockercompose + auto_deploy=false + docker_compose_domains)
DOMAINS_JSON='[{"name":"'"$MAIN_SVC"'","domain":"https://'"$APP_NAME"'.tranfu.com:'"$MAIN_PORT"'"}]'

APP_UUID=$(curl -sS -X POST \
  -H "Authorization: Bearer $COOLIFY_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d "$(jq -nc \
    --arg n "$APP_NAME" \
...
curl -sS -H "Authorization: Bearer $COOLIFY_API_TOKEN" "$BASE/api/v1/applications/$APP_UUID" \
  | jq '{uuid, name, build_pack, is_auto_deploy_enabled, github_app_uuid, git_repository, git_branch}'
```

详见 [../commands/application-crud.md](../commands/application-crud.md) §"创建 Application"。

**4I.4 校验**: 必须满足 `build_pack == "dockercompose"` && `is_auto_deploy_enabled == false` && `github_app_uuid == $GITHUB_APP_UUID` && `git_repository == "$REPO_ORG/$REPO_NAME"`。任一不对 → 终止 + 报告字段不一致 (大概率是 POST body 字段写错)。
...
✓ 已创建 Coolify 资源:
  ...
  Application: $APP_NAME ($APP_UUID)
  绑定:        $REPO_ORG/$REPO_NAME @ $DEFAULT_BRANCH (via GitHub App $GITHUB_APP_UUID)
  build_pack:  dockercompose
  auto_deploy: false (GitHub push 不会触发 Coolify 自动部署)
  域名:        https://$APP_NAME.tranfu.com:$MAIN_PORT
  Coolify URL: $BASE/applications/$APP_UUID
  接下来 GH workflow 会用到 COOLIFY_APP_UUID = $APP_UUID, 写入 GH environment vars (Step 5I)。
```

**Act 失败**：

- 400 missing required → 检查 `$PROJECT_UUID` / `$SERVER_UUID` / `$GITHUB_APP_UUID` 真存在
- 404 GitHub App not found → `$GITHUB_APP_UUID` 错或被删, 重跑 preflight
- 404 repository not found → GitHub App 没在该 repo 装 Installation, 去 github.com 给 GitHub App 加 repo access
- 409 domain conflict → `docker_compose_domains` 里某条已被别的资源占用，[../commands/domain.md §"域名冲突"](../commands/domain.md)
- 422 → 重点查 `docker_compose_domains` 是不是 `[{name, domain}]` 数组 (不是 `[{name, url}]`, 那是旧 Service 形态)
...
gh variable set COOLIFY_APP_UUID     --env "$DEFAULT_BRANCH" --body "$APP_UUID"
...
### Step 6I: application env 变量

仓库根有 `.env` → 逐条 POST `/api/v1/applications/$APP_UUID/envs`（详见 [../commands/application-env.md](../commands/application-env.md)）。
...
- **必须按 [../commands/application-env.md](../commands/application-env.md) 的 grep filter 过滤 Coolify 魔法变量前缀** (compose 声明的 `SERVICE_PASSWORD_*` / `SERVICE_HEX_*` / `SERVICE_FQDN_*` 等, Coolify 自己注入, 重复 POST 会冲突)
...
## 更新部署分支（Step 2U, Application 已存在）

**乐观假设**：Application 已存在 = 已部署好。agent **不主动核对源码 / 不主动改文件 / 不主动 push** — 只按用户原话提炼的意图执行最小 act, 共用收尾自动验。
...
|---|---|---|---|
| `redeploy` / 重新部署 / 重启 / 重新拉镜像 | **A** | POST `/api/v1/deploy?uuid=$APP_UUID&force=false` | [../commands/deploy-trigger.md](../commands/deploy-trigger.md) §"手工触发部署" |
| 改 / 加 / 删域名 | **B** | PATCH `/api/v1/applications/$APP_UUID` 改 `docker_compose_domains` 数组 | [../commands/domain.md](../commands/domain.md) |
| 改 / 加 / 删 env 变量 | **C** | POST/PATCH `/api/v1/applications/$APP_UUID/envs` → 后接 A (env 改了必须 redeploy 才生效); 按 application-env.md 的 grep filter 跳过魔法变量前缀 | [../commands/application-env.md](../commands/application-env.md) |
| 改 compose / Dockerfile / 改源码 / 改 deploy.yml | **D** | `mktemp -d` clone → spawn subagent 按 [../references/file-generation-rules.md](../references/file-generation-rules.md) 改 → autonomous push。**compose 走 git binding, 不需要 PATCH `docker_compose_raw` 字段** (那是旧 Service 形态); Coolify 下次 deploy 会从 git 读最新 commit 的 compose | [../commands/application-crud.md](../commands/application-crud.md) §"更新 Application" |
...
**D 路径关键变化 (相比 0.7)**: 0.7 Service 形态下, 改完 compose 需要 PATCH `docker_compose_raw` 字段 (因为 compose 存在 Coolify 里); 0.8 Application + GitHub App 形态下, compose 存在 git repo 里, **改完 push 即可**, Coolify 触发 deploy 时自己读最新 commit 的 compose, 不需要任何 PATCH 调用。
...
"markdown-kits-app 的 compose 加个 redis"          → D (push 即可, 无 PATCH)
"markdown-kits-app 改完域名再重启一下"             → B + A
```

**不在 Step 2U 做**：

- 不主动核对四件套合规 (Application 已存在 = 假设当时合规, 不重审)
- 不主动核对 compose 跟 git binding 一致性 (假设当时已对齐, 后续若不一致由 D 路径用户主动触发)
...
- 30s 内启动 → ✓
- 未启动 + 手动 POST 200 → ✓（GHA 那一步 POST /deploy 失灵，但 fallback 救了）
...
FQDN=$(curl -sS -H "Authorization: Bearer $COOLIFY_API_TOKEN" \
  "$BASE/api/v1/applications/$APP_UUID" | jq -r '.applications[0].fqdn')
...
| 404 | traefik 没收到这个域名 | 检查 `docker_compose_domains` 是否同步, 重跑更新分支 B |
| 5xx (非 502) | 应用内部错 | Coolify application logs / 应用代码 |
...
- **不要忽略 Step 1 入口分流**：Application 已存在还跑 Step 2I clone 是浪费 + 跟用户预期不一致
- **不要在用户 cwd 直接 clone**：Step 2I / 更新分支 D 路径必须 `mktemp -d`，用完即弃 — **永远不要求 user "先 cd 到 repo"**
- **更新分支不要主动核对源码 / 不要主动 push**：Application 已存在 = 乐观假设已部署好, 只处理用户指派的任务; 主动 PR 一堆"我顺手改了你的 Dockerfile"是事故
...
- **Step 1 不要只按 name filter Application**：必须 `name + project_uuid` 双重 filter (防跨 project 同名误命中)
...
- **不要忘记 `is_auto_deploy_enabled: false`**：4I.3 POST body 漏这个字段, Coolify 默认会订阅 GitHub push webhook 自己 build, 跟 GHA 链路会撞 (镜像版本可能不一致); 4I.4 GET 校验里必须确认这个字段 = false
- **不要在 4I.3 创建 Application 时漏 `build_pack: "dockercompose"`**：默认 build_pack 可能是 nixpacks, 会导致 Coolify 走完全错的部署模式
- **不要 PATCH `docker_compose_raw` 字段** (更新分支 D 路径)：那是旧 0.7 Service 形态。0.8 Application + GitHub App 形态下, compose 走 git, 改完 push 即可, 不需要 PATCH 任何字段
- **不要 PATCH `urls` 字段** (更新分支 B 路径): 那是旧 Service 形态, Application endpoint 不接受, 会 422; 用 `docker_compose_domains`
- **不要主动碰 Service namespace**: 同名 Service 残留是 Step 1 的边界 case (告知后终止), reconcile 主流程不读不写 `/api/v1/services/*`
