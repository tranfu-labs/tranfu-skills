...
If no production source exists, ask for the missing brief before producing.
...
1. Resolve the production source using the `Input Priority` section.
...
7. Create a new medium-specific visual system from the production source, new brief, real target context, and optional brand context.
...
- If no production source exists, ask for a brief and stop.
...
- If the user requests source-brand copying, refuse that part and create an original output from abstract DNA.
...
4. Brief statement of production source: `Producer Handoff`, `Visual DNA`, or `standalone brief`.
...
- The output copies the source identity instead of using abstract visual DNA.
- Provided real target context was ignored.
- The output requires a missing brand path in default `brandless` mode.
- The output uses source logos, source assets, exact layouts, proprietary components, or exact copy.
...
Action: Activate `brand-on`, read the provided brand guide, blend target brand with abstract DNA, avoid source identity copying, and produce HTML.
</example>
...
Reason: This copies source identity. Produce an original artifact from abstract visual principles.
</bad-example>
...
- The output uses Visual DNA abstractly when Visual DNA is present.
- `brandless` mode works without any brand path.
- Optional `brand-on` mode is available when brand context is provided.
- Source identity, exact layouts, proprietary components, and exact copy are not reused.
