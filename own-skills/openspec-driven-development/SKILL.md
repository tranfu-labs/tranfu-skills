...
description: >-
  ...
  归档（移动 change、合并 spec-delta、若有 wireframes.md 则回流到 docs/wireframes/）→ 更新 AGENTS.md → git commit → 有 remote 则询问 push」的闭环。
...
步骤2 聊天框出方案（字符图 / 测试用例）──确认──> 步骤3
   │
步骤3 落盘 openspec/changes/<change>/（含必要时的 wireframes.md）
   │
...
3. 步骤2：聊天框出方案——新增 `scripts/export-all.mjs`（遍历系列、复用单卡导出逻辑），改 `app.html` 加一个按钮（**有视觉变化 → 附字符图，基线引用 `docs/wireframes/pages/app.md`**）。导出逻辑含可测的文件名生成 / 系列遍历（**可测逻辑 → 附单测用例**：文件名生成、空系列、部分失败），再附 AI 验证流程（playwright 跑一遍导出、截图核对产物数量）。用户确认。
4. 步骤3：写入 `openspec/changes/add-export-all/`（proposal + design + tasks + spec-delta + **wireframes.md 含字符图**）。
5. 步骤4：反思方案，发现没定义"部分失败时是否继续"，回去问用户、补进方案。问「是否开始写代码？」用户：「开始写代码。」
6. 步骤5-6：按 spec 实现，单测绿、playwright 验证导出数量正确。
7. 步骤7：按 `openspec/changes/AGENTS.md` 的「归档」节三步并列——① `add-export-all/` 移到 `archive/2026-06-20-add-export-all/`；② 导出能力的 delta spec 合进 `openspec/specs/export/`；③ `wireframes.md` 的字符图回流到 `docs/wireframes/pages/app.md`（按钮变更同步进 `flow.md` 相关流程节）。
