...
description: >-
  ...
  从最新 main 切 feature 分支并落盘 openspec/changes → 按 spec 写码 → 反思代码符合度 →
...
- **「符合」消歧**：问「代码是否符合**方案 / spec / 某个 change**」→ 这是步骤 5，触发；问「项目是否符合**规范 / 标准 / 约定**」这类泛合规审计 → 与具体改动无关，不触发。
...
步骤0 识别开发任务 + openspec 检测 + 进场路由
   ...
   └─有 openspec，按开场分流：
       "实施 openspec/changes/X" ───────> 直接进 步骤4
       "实施完了，检查是否符合方案/遗漏" ─> 直接进 步骤5
       微改动 ─────────────────────────> 步骤1 快车道
...
步骤1 采访 / 诊断澄清目标
   ...
   └─结论：需要改代码────> 步骤2
   │
步骤2 聊天框出方案（字符图 / 测试用例）→ agent 自审 → 默认自动到步骤3
   │（若推进计划指定停在 plan-confirmed，则等用户拍板）
步骤3 检测/切到 feature 分支（NEVER 在 main/master 写入）→ 落盘 openspec/changes/<change>/
   │（若推进计划指定停在 code-greenlit，则问"是否开始写代码？"等用户拍板）
步骤4 按 spec 写代码（在 feature 分支上）
   │
步骤5 反思代码符合度 ──有偏差──> 回步骤4 改
   │
步骤6 归档（按 openspec/changes/AGENTS.md 的「归档」节）：
   ...
   │
步骤7 更新 AGENTS.md → git commit（feature 分支）→ 有 remote 则 push + 开 PR（body 粘 proposal.md）
```

CREATE A TODO LIST 覆盖步骤 0-7，每步做完更新状态。**MUST 在步骤 0 末把推进计划（见下节）亮给用户、把"在哪闸门停"接住，再开干**；除非推进计划里写了停在 `plan-confirmed` / `code-greenlit`，否则这两个闸门默认自动通过。

---

## 自动推进与闸门枚举（v0.4 起默认）

**默认全自动衔接**：步骤 0 一旦确定进场点，整条流水线自动往下推进、不再每步问"要不要继续"。**唯一会天然停下等用户的是步骤 1 采访 / 诊断**——因为对话往返本身是闸门兑现过程，没法跳过。

其余所有节点（含 `plan-confirmed`、`code-greenlit`、归档、commit）**默认 agent 自审通过后直接进入下一步**——它们仍是稳定锚点，但是否要变成"等用户拍板"由步骤 0 的推进计划决定，不再默认硬等。

最后的提 PR（`pr-opened`）是个特殊节点：**有 remote 时始终去做**（push feature 分支 + `gh pr create`），不再"问用户要不要推"——因为 PR 本身就是给 reviewer 的介入点，不该被绕开。但 **NEVER 自己合并 PR**，合并由 reviewer 拍板。

### 闸门枚举表

每一步结尾都对应一个**闸门枚举名**（kebab-case），给外部（用户在 prompt 里、上游 skill、流程编排）一个稳定锚点，可指定「跑到某个闸门就停」或「在某个闸门额外做点什么」。

| 闸门 | 含义 | 步骤结尾 | 默认行为 |
|---|---|---|---|
| `intake-routed` | 进场点已确定 | 步骤 0 | 自动 |
| `interview-confirmed` | 采访 / 诊断完成，目标明确（结论"不改代码"在此干净结束） | 步骤 1 | **天然等用户**（采访对话不可省） |
| `plan-drafted` | 聊天框方案已成形 | 步骤 2 输出后 | 自动 |
| `plan-confirmed` | 方案审核通过——默认 agent 自审认可；prompt 指定停时改为等用户拍板 | 步骤 2 末 | 自动（可被指定为停） |
| `branch-checked-out` | 已在为本 change 准备的 feature 分支上（新切 or 复用已有的）| 步骤 3 开头 | 自动；当前已在合适分支则跳过 |
| `plan-persisted` | 方案落盘到 `openspec/changes/<change>/` | 步骤 3 | 自动 |
| `code-greenlit` | 进入写码阶段——默认 agent 自审即放行；prompt 指定停时改为等用户口令 | 步骤 3 末 → 步骤 4 之间 | 自动（可被指定为停） |
| `code-written` | 按 spec 的代码（含落地测试）写完 | 步骤 4 | 自动 |
| `code-verified` | 代码符合度反思通过、测试 / AI 验证流程跑通 | 步骤 5 | 自动 |
| `archived` | 归档完成（移目录 + 合 spec-delta + 必要时回流 wireframes） | 步骤 6 | 自动 |
| `committed` | `git commit` 到 feature 分支完成 | 步骤 7 | 自动 |
| `pr-opened` | PR 已创建（push feature 分支 + `gh pr create`，body 粘 proposal.md）| 步骤 7 末 | **有 remote 时始终走 PR**（不直接 push 主干）；无 remote 则跳过 |

### 推进计划的显示模板

**步骤 0 一定下进场点，MUST 先在对话里明示推进计划，再开干**——这是 `intake-routed` 闸门的兑现动作，也让用户最后一次机会指定在哪儿停 / 加事。三种形态：

**A. 默认（全自动衔接）**：

> 推进计划（每步自动衔接；步骤 1 采访 / 诊断会自然停下等你回答；最后有 remote 会推 feature 分支并开 PR）：
> `intake-routed → interview-confirmed → plan-drafted → plan-confirmed → branch-checked-out → plan-persisted → code-greenlit → code-written → code-verified → archived → committed → pr-opened`

**B. 用户指定停在某闸门**（例：prompt 里写「方案给我看一眼再写码」）：

> 推进计划（按你要求在 `code-greenlit` 前停下，由你拍板再写码；停下前每步自动衔接）：
> `intake-routed → interview-confirmed → plan-drafted → plan-confirmed → branch-checked-out → plan-persisted ⏹停⏹ code-greenlit → code-written → code-verified → archived → committed → pr-opened`

**C. 用户指定在某闸门额外做事**（例：「在 `plan-confirmed` 时同步把方案贴到 Slack」）：

> 推进计划（在 `plan-confirmed` 触发额外动作：把方案贴 Slack）：
> `intake-routed → interview-confirmed → plan-drafted → plan-confirmed ⨁[贴 Slack] → branch-checked-out → plan-persisted → code-greenlit → code-written → code-verified → archived → committed → pr-opened`

形态可组合（既指定停、又指定额外动作）。符号 `⏹` / `⨁` 不是硬约定——核心是**把闸门枚举名写清楚**，让用户能精确指认是哪一步。

> 为什么默认自动而不是逐步问：原来每一步都问"要不要继续"，对一条已经达成共识的链路是冗余，最终演成"用户疯狂回车"。采访（`interview-confirmed`）是 agent 没法替代的对话往返，必然停；其他节点 agent 能自审，默认就别打扰用户。需要在 `plan-confirmed` / `code-greenlit` 等用户拍板的，由用户在 prompt 里显式指定即可。
> 为什么要枚举名：让外部能精确指认"在哪儿停 / 在哪儿加事"。"在第 3 步停"是脆弱的（步骤编号会随版本漂移），"在 `plan-persisted` 停"是稳定的。
...
| 开场判据 | 进场点 |
|---|---|
| `实施 openspec/changes/<X>`（spec 已写好，只差实现）| 直接进 **步骤 4** |
| `实施完了 / 已完成，检查是否符合方案、有无遗漏` | 直接进 **步骤 5** |
...
**步骤 0 末必做（兑现 `intake-routed`）**：进场点定下后，**在对话里明示推进计划**（按上节「推进计划的显示模板」给出 A / B / C 形态之一）——把整条闸门链亮出来，告诉用户哪些自动跑、哪些会停、是否带额外动作。这是给用户的最后一次「指定停在哪儿 / 加事」机会，**NEVER 跳过这一步直接进入步骤 1**。
...
- 仍走**步骤 5**（代码反思，对照用户口头需求核一遍）。
- **跳过步骤 6**：微改动没有 `openspec/changes` 文档可归档，不强行造一个。
- **步骤 7 照走**：有受影响的文档就更新 → `git commit`（feature 分支） → 有 remote 则推分支并开 PR；快车道**没有 proposal.md**，PR body 用 commit message + 一两句口头描述补充。
...
- **默认（推进计划没指定停在 `plan-confirmed`）**：agent 以审稿人视角自审一遍，没问题就视为 plan-confirmed，**直接进入步骤 3**；有不确定项才问用户。
- **推进计划指定停在 `plan-confirmed`**：聊天框出完方案后，明确请用户拍板，得到确认再进步骤 3。
...
### 3a. 检测当前分支状态（兑现 `branch-checked-out`）

`plan-confirmed` 后，第一件事是 `git status` / `git branch --show-current`，按下表分流：
...
`plan-persisted` 后到了写码闸门 `code-greenlit`：

- **默认（推进计划没指定停在 `code-greenlit`）**：直接进入写码，不再单独问用户「是否开始写代码？」。
- **推进计划指定停在 `code-greenlit`**：明确询问「是否开始写代码？」，等用户拍板。
  - 用户说**「开始写代码」**（或等价明确指令）→ 进入写码。
  - 用户提了别的事 → 按新指令做；该排期的新需求记一条**临时插入的 TODO**，别丢。

> **若从步骤 0 直接进来（「实施 openspec/changes/X」）**：先确认 `openspec/changes/<X>/` 确实存在且方案完整——它存在意味着方案此前已被确认过，可直接实现；**不存在或残缺 → 回退步骤 1/2** 先把方案补全、确认，再实现。

严格按 `openspec/changes/<change>/` 里的方案写。方案里定义了的测试用例，这一步要真正落地成可运行的测试。如果写码过程中发现方案有硬伤，停下回步骤 2 修方案再继续——别将错就错。
...
- 有偏差 / 遗漏 → 回步骤 4 补齐。
- 完全符合、测试通过（单测绿 / AI 验证流程跑通）→ 进入步骤 6。
...
- 检查 remote：
  - **有 remote**：始终走 PR，**不再 "问是否 push"**——
    ...
    2. `gh pr create --title "<commit message 第一行>" --body "$(cat openspec/changes/archive/<YYYY-MM-DD>-<change-id>/proposal.md)"`——**PR body 直接粘 proposal.md**，因为 proposal 本来就是 why / what / impact，正好是 PR 描述，无需二次编辑。
    3. 把 PR URL 回报给用户。
...
- **没出方案就被要求写码** → 没有 `plan-drafted` 就不可能写码。提醒用户先出方案、回步骤 2；除非用户明确说"跳过方案直接写"。（注：`plan-confirmed` / `code-greenlit` 默认 agent 自审通过，不需要用户每次拍板——这是"自动推进"的设计，不是"绕过方案"）
...
- NEVER 跳过方案直接写代码——`plan-drafted` 必经，方案必须落盘到 `openspec/changes/`。
- NEVER 在推进计划里指定停在 `code-greenlit` 的情况下，用户没说"开始写代码"就动代码。
- NEVER 在没有 openspec 的项目里自己搭骨架——停下引导 project-init-docs。
- NEVER 因"不方便单测"就跳过测试——改架构使其可测。
- NEVER 漏掉闭环收尾：写完代码必须按步骤 6 完成归档——移动 change、合并 spec-delta、**有 `wireframes.md` 时必须回流到 `docs/wireframes/`**——再走步骤 7 更新 AGENTS.md。漏回流 wireframes 等于让版式事实源失真，与漏合并 spec-delta 同等严重。
...
- NEVER 把"自动推进"理解成"连步骤 1 采访也跳过"——采访的对话往返本身是 `interview-confirmed` 闸门的兑现过程，没法替用户答。
...
3. 步骤2：聊天框出方案——新增 `scripts/export-all.mjs`（遍历系列、复用单卡导出逻辑），改 `app.html` 加一个按钮（**有视觉变化 → 附字符图，基线引用 `docs/wireframes/pages/app.md`**）。导出逻辑含可测的文件名生成 / 系列遍历（**可测逻辑 → 附单测用例**：文件名生成、空系列、部分失败），再附 AI 验证流程（playwright 跑一遍导出、截图核对产物数量）。agent 自审无遗漏 → `plan-confirmed` 自动通过。
4. 步骤3：先做分支检测——`git branch --show-current` 显示在 `main` → `git fetch origin && git checkout -b change/add-export-all origin/main`。再写入 `openspec/changes/add-export-all/`（proposal + design + tasks + spec-delta + **wireframes.md 含字符图**）。推进计划默认没指定停在 `code-greenlit` → 自动放行进入步骤 4。
5. 步骤4-5：按 spec 实现，单测绿、playwright 验证导出数量正确。
6. 步骤6：按 `openspec/changes/AGENTS.md` 的「归档」节三步并列——① `add-export-all/` 移到 `archive/2026-06-29-add-export-all/`；② 导出能力的 delta spec 合进 `openspec/specs/export/`；③ `wireframes.md` 的字符图回流到 `docs/wireframes/pages/app.md`（按钮变更同步进 `flow.md` 相关流程节）。
7. 步骤7：更新 AGENTS.md 的脚本清单 → `git commit` 到 `change/add-export-all` 分支 → `git push -u origin change/add-export-all` → `gh pr create --title "<commit message 第一行>" --body "$(cat openspec/changes/archive/2026-06-29-add-export-all/proposal.md)"` → 把 PR URL 回报给用户。**不自己合并**。
</example>
...
1. 步骤0：是 bug 讨论、仓库有 `openspec/` → 路由进 **步骤 1**（不是从需求起步）。用户 prompt 里说「先讨论清楚」≈ 跑到 `interview-confirmed` 就停，**明示推进计划（B 形态）**：
   ...
   > `intake-routed → interview-confirmed ⏹停⏹ plan-drafted → plan-confirmed → branch-checked-out → plan-persisted → code-greenlit → ...`
...
- 落盘后又问「要我开始写代码吗？」等用户回。
- 每个反思代码 / 归档 / commit 环节都停下来问「要我继续吗？」

为什么错：用户没指定停在 `plan-confirmed` / `code-greenlit`，按推进计划默认形态就该 agent 自审通过后自动推进，硬等用户拍板是把"自动推进"重新降级回 v0.3 的"逐步问"。**真要让用户审，让他在 prompt 里写「停在 plan-confirmed」**——把控制权交给推进计划，而不是 agent 替用户做"安全起见还是问一下"的选择。**唯一该天然停的是**：步骤 1 采访本身（对话往返必然停）。最后开 PR 这一步则始终走（不是"停"，是"始终去做"）——直接推 main 反而是错的。
</bad-example>
