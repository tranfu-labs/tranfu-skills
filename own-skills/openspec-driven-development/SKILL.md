...
步骤7 归档（按 openspec/changes/AGENTS.md 的「归档」节）：
   ...
   │
步骤8 更新 AGENTS.md → git commit（feature 分支）→ 有 remote 则 push + 开 PR（body 粘 proposal.md）
```

CREATE A TODO LIST 覆盖步骤 0-8，每步做完更新状态。**MUST 在步骤 0 末把推进计划（见下节）亮给用户、把"在哪闸门停"接住，再开干**；除非推进计划里写了停在 `plan-confirmed` / `code-greenlit`，否则这两个闸门默认自动通过。
...
| `plan-reviewed` | 方案反思完成（已修补遗漏 / 问完不确定项） | 步骤 4 | 自动 |
| `code-greenlit` | 进入写码阶段——默认 agent 自审即放行；prompt 指定停时改为等用户口令 | 步骤 4 末 | 自动（可被指定为停） |
| `code-written` | 按 spec 的代码（含落地测试）写完 | 步骤 5 | 自动 |
| `code-verified` | 代码符合度反思通过、测试 / AI 验证流程跑通 | 步骤 6 | 自动 |
| `archived` | 归档完成（移目录 + 合 spec-delta + 必要时回流 wireframes） | 步骤 7 | 自动 |
| `committed` | `git commit` 到 feature 分支完成 | 步骤 8 | 自动 |
| `pr-opened` | PR 已创建（push feature 分支 + `gh pr create`，body 粘 proposal.md）| 步骤 8 末 | **有 remote 时始终走 PR**（不直接 push 主干）；无 remote 则跳过 |
...
> `intake-routed → interview-confirmed → plan-drafted → plan-confirmed → branch-checked-out → plan-persisted → plan-reviewed → code-greenlit → code-written → code-verified → archived → committed → pr-opened`

**B. 用户指定停在某闸门**（例：prompt 里写「方案给我看一眼再写码」）：

> 推进计划（按你要求在 `code-greenlit` 前停下，由你拍板再写码；停下前每步自动衔接）：
> `intake-routed → interview-confirmed → plan-drafted → plan-confirmed → branch-checked-out → plan-persisted → plan-reviewed ⏹停⏹ code-greenlit → code-written → code-verified → archived → committed → pr-opened`

**C. 用户指定在某闸门额外做事**（例：「在 `plan-confirmed` 时同步把方案贴到 Slack」）：

> 推进计划（在 `plan-confirmed` 触发额外动作：把方案贴 Slack）：
> `intake-routed → interview-confirmed → plan-drafted → plan-confirmed ⨁[贴 Slack] → branch-checked-out → plan-persisted → plan-reviewed → code-greenlit → code-written → code-verified → archived → committed → pr-opened`
...
- 有遗漏 → 直接改方案。
- 有不确定项 → 问用户，别自己拍板。
- 没问题 / 问完了 → 视为 `plan-reviewed` 闸门通过。

`code-greenlit` 闸门：

- **默认（推进计划没指定停在 `code-greenlit`）**：`plan-reviewed` 通过后自动放行进入步骤 5，不再单独问用户「是否开始写代码？」。
- **推进计划指定停在 `code-greenlit`**：明确询问「是否开始写代码？」，等用户拍板。
  - 用户说**「开始写代码」**（或等价明确指令）→ 进入步骤 5。
  - 用户提了别的事 → 按新指令做；该排期的新需求记一条**临时插入的 TODO**，别丢。

> 为什么默认不再硬等：之前每次都强行问"是否开始写代码"，对一条已经走过采访 + 方案确认的链路是冗余。把"什么时候让用户介入"交给步骤 0 的推进计划——用户想看就指定停在 `code-greenlit`，不想看就不指定。
> 默认仍要保留 `code-greenlit` 这个枚举名：它是写码许可的稳定锚点，外部要在这儿挂事（比如 CI 触发、通知）依然能精确指认。
...
5. 步骤4：反思方案，发现没定义"部分失败时是否继续"——这是不确定项，**回去问用户**、得到回答后补进方案，再过 `plan-reviewed`。推进计划默认没指定停在 `code-greenlit` → 自动放行进入步骤 5。
...
8. 步骤8：更新 AGENTS.md 的脚本清单 → `git commit` 到 `change/add-export-all` 分支 → `git push -u origin change/add-export-all` → `gh pr create --title "<commit message 第一行>" --body "$(cat openspec/changes/archive/2026-06-29-add-export-all/proposal.md)"` → 把 PR URL 回报给用户。**不自己合并**。
</example>
...
1. 步骤0：是 bug 讨论、仓库有 `openspec/` → 路由进 **步骤 1**（不是从需求起步）。用户 prompt 里说「先讨论清楚」≈ 跑到 `interview-confirmed` 就停，**明示推进计划（B 形态）**：
   ...
   > `intake-routed → interview-confirmed ⏹停⏹ plan-drafted → plan-confirmed → plan-persisted → ...`
