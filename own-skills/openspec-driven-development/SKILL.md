...
**步骤 1 采访 / 诊断是 HARD STOP**——必须用户回答、agent 不能自审通过、不能替答、不能跳过。对话往返本身是闸门兑现过程，没法绕过。

其余所有节点（`plan-confirmed`、`code-greenlit`、归档、commit 等）**默认 agent 自审通过后自动衔接进入下一步**，不再每步问"要不要继续"——它们仍是稳定锚点，但是否要变成"等用户拍板"由步骤 0 的推进计划决定，不再默认硬等。
