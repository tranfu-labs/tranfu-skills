...
**测试前先构建**：deploy.yml 加测试卡口时，若项目有独立 build 命令、且测试用例消费构建产物（典型场景：monorepo 子包跨包 import 编译出的 `dist`；前端 build 出 `dist` 给后端测试读），必须把 build step 排在 test step 之前。本地能跑通往往只是本地早 build 过的副作用，**只有 GHA 这条"干净 checkout"的链路会暴露**，首次撞到容易误判成依赖装不上。
