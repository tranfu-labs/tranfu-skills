...
#### 2a. 先做验收落点表(拆任务的输入,不是产出)

CRITICAL: 在抛出任何候选任务之前,MUST 先把 PRD 的验收标准**横着摊开**,逐条回答"这条最终由哪个持久化字段 / 哪个进程边界 / 哪个模块承载"。这一步只能一次性看全,拆成任务之后就再也没人做了。
...
| --- | --- | --- |
| #8  | 上次使用团队记录文件 + 新建对话预选读取 | 已存在 |
| #10 | 会话表的团队列 + IPC 透传 + 会话视图发送闸门 | **不存在** |
```

判定规则:

- **落点已存在** → 正常拆进实现任务
- **落点不存在** → MUST 先成为一个独立的前置任务,并成为所有依赖它的任务的 `depends_on`。NEVER 把"落点不存在"的验收直接写成某个界面任务的子勾选项
- **写不出落点** → 这条验收要么 PRD 表述不可实现,要么缺前置设计,MUST 提出来让用户裁决,NEVER 含糊带过

判定的关键提问是:**"这条验收所说的状态,现在存在于哪里?"** 如果答案是"要新建一个字段/表列/接口",那它就是一个任务;如果答案含糊成"界面上那个选中的东西",几乎一定是执行者会把它接到手边最像的现有状态上——这正是"任务打勾但功能没实现"的产生机制。

落点表 MUST 随 Step 3 写进 `design.md`,让每个任务执行时都能回查。
...
- **design.md**: **验收落点表(Step 2a 的产出,放最前面)** / 方案(执行编排 + 关键决策 3-5 条) / 权衡(选这个方案放弃了什么) / 风险(已知风险与回滚思路)
- **tasks.md**: 按 openspec 惯例的嵌套 checkbox,顶层 T1..TN,子勾选项写子步骤和"覆盖验收 #X #Y"。**子勾选项落盘时一律留空 `[ ]`,勾选由主 loop 在落地成功后连同证据锚一起写入(见 loop-instructions 第 3 步),NEVER 由执行者自行勾选**
- **spec-delta/<domain>/spec.md**: 落盘时**只建骨架**(文件头 + 域名),不预写 Requirement

关于 spec-delta,本 skill 采用**逐任务追加**而非开头一次性写完:

- 每个任务在完成时追加**自己那份** Requirement,由主 loop 在落地时合并回 `openspec/specs/<domain>/spec.md`
- 理由:开头一次性写时,写的人面对全部任务的全景,只会挑"最像规格"的少数几条落成 Requirement,其余验收永久失去回归护栏。逐任务追加则每个任务都被迫产出自己那份,覆盖率不会漏
- 每个 Requirement MUST 带一行 `Source: <PRD 相对路径>#<小节>` 指回产品事实源,只写可判定的判据(MUST / MUST NOT + Scenario),NEVER 复述 PRD 的决策理由

若项目的 `openspec/changes/AGENTS.md` 明文规定 spec-delta 必须等整个 change 归档才合并,**以项目规则为准**,但要向用户指出这条规则的代价:先完成的任务其行为已是既成事实,让它等最后一个任务才登记,中间所有任务都在对着过时的 specs 工作,跨任务的契约漂移由此产生。建议用户把该规则的粒度从 change 改成 task。
...
WRONG: 把一条"落点不存在"的验收直接写成某个界面任务的子勾选项
Reason: 例如验收写"已有会话选了需要修复的团队时阻止发送",而会话记录里根本没有团队字段。执行者在自己的 worktree 里只看得到手边的现有状态,就会把它接到界面上那个同名的选择态上——子勾选项能打勾、测试能写绿,功能是错的。落点不存在时 MUST 先拆出独立的前置任务,见 Step 2a。

WRONG: 在 change 创建时把 spec-delta 一次性写完(尤其是"写个 1-2 条 Requirement 意思一下")
Reason: 面对全部任务的全景写规格,只会挑最像规格的少数几条落笔,其余验收永久没有回归护栏。实测后果是十余条验收只落了四条 Requirement,没落的那些行为(滚动位置恢复、返回按钮拦截等)既无规格约束也无测试覆盖。spec-delta MUST 逐任务追加。
...
2a. **`design.md` 含验收落点表,且行数 = PRD「指标与验收」的条数**——一条不漏。缺行即为漏拆,MUST 回到 Step 2a 补齐再落盘
2b. **PRD 的每个验收编号至少被一个任务的 `acceptance_ids` 覆盖**——把全部任务的 `acceptance_ids` 取并集,与 PRD 验收编号全集比对,缺一个都不算成功
2c. **落点表里标记为「不存在」的落点,每个都对应一个独立的前置任务**,且依赖它的任务已把它写进 `depends_on`
