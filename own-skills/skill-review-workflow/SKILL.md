...
- 产出命名产物 `SKILL_REVIEW_REPORT`, 含四门结论 (短路退出时未跑的门标 `not-run`)、结构化 issue 列表、三态总裁决
- 每条 issue 带 severity / 命中检查项 / 行锚点或段落锚点 / 证据 / 建议改法
- 门 3 的合并 `REVIEW_PACKET` 覆盖全部已审文件, 且包内 issue id 唯一 (id 已顺移)
...
8. T5: 合并 (id 顺移)、机械裁决, 产出 `SKILL_REVIEW_REPORT`, 结束。
...
短路退出的统一动作 (T1 / T2 / T3 的卫语句都指到这里): 跳到步骤 8, 用已回收的产出组装局部 `SKILL_REVIEW_REPORT`——已跑的门带完整结果, 未跑的门标 `result: not-run, mode: skipped-short-circuit`, verdict 按 §6 短路规则取值, 然后结束。短路退出仍是正常完成, 不是失败路径。
...
- 含多个 skill 的父目录 (如 `own-skills/`) → batch 模式: 对每个子 skill 独立执行步骤 3-8, 最后汇总一张 per-skill 裁决表; 单个 skill 审查失败或短路退出 NEVER 中断其余 skill。
...
| T5 | (主 agent, 非 SubAgent) 合并裁决 | — | 全部已回收产出 | 串行 | T4 完成, 或任一短路点跳入 | `SKILL_REVIEW_REPORT` |
...
回收判定: 返回 `通过` → gate1: pass, 进入 T2。返回 `打回` → gate1: fail, 记录缺失字段 (这不是措辞问题, 是 "该内容不够格成为 skill" 的信号, 报告中附 "降级为普通文档" 选项), **短路退出**。
...
回收判定: 当前名 == Top1 → gate2: pass, 进入 T3。当前名 ≠ Top1 且 (Top1 总分 − 当前名总分) ≥ 2 → gate2: fail (容器框错, 附 Top1 作为改名或拆分建议), **短路退出**。当前名 ≠ Top1 且分差 < 2 → gate2: pass, 附 note "存在同分级替代容器 {Top1}, 不构成重构理由", 进入 T3。
...
2. 合并门 3: 跨文件合并为一个 `REVIEW_PACKET`, 文件顺序 = `PROMPT_FILES` 顺序 (SKILL.md 首位)。id 顺移指的是**审核结果中每条 issue 的 id** (维度字母+序号): 各片段内 id 都从 1 起编, 合并时对同维度字母的 issue id 按文件顺序重新连续编号 (如 SKILL.md 片段有 C1、C2, 下一文件片段的 C1 顺移为 C3), 每条 issue 的 `location` MUST 含文件路径, 最后重算 ledger。合并后包内 issue id MUST 唯一。
...
- SubAgent 无产出或产出不合法 → 同模板重派一次; 再失败 → 该缺口 `mode: degraded`, 主 agent 本地补审并记入 `residual_risks`; 短路判定点 MUST 补齐结论后再决定去留。
...
6. T3: SKILL.md × 3 reviewer + 门 4 并发。SKILL.md 片段仅 1 条 MEDIUM (C1), 无 BLOCKER/HIGH → 不短路; 门 4 缺 `<bad-example>` (HIGH)。进入 T4。
7. T4: agents/openai.yaml × 3 reviewer, 回收 1 条 MEDIUM (C1)。
8. T5: 合并——openai.yaml 的 C1 顺移为 C2, ledger 重算; gate3 pass (无 BLOCKER/HIGH), gate4 fail (HIGH)。裁决: `verdict: 需修改`, `short_circuited_at: null`。产出 SKILL_REVIEW_REPORT, 零文件改动。

Workflow (T1 短路的情形): 同一目标若门 1 返回 `打回` (如内容只是项目状态记录), 则跳过 T2-T4, 直接产出局部报告: gate1 fail 带缺失字段, 其余三门 `result: not-run, mode: skipped-short-circuit`, `short_circuited_at: T1`, `verdict: 建议重构`, 结束——全程只消耗 2 个 SubAgent。
</example>
...
<bad-example>
WRONG: 为了省等待时间, T1 时把四门共 9 个 SubAgent 全部一次派完; 门 1 回来是 `打回`。

Reason: 违反分级短路——内容准入已判死, 后面 7 个 SubAgent 的 token 全部白烧, 且其结论会诱导报告在 "该不该存在" 之外堆砌措辞级建议, 稀释 `建议重构` 的信号。
</bad-example>
