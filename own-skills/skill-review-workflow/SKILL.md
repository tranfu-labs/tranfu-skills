...
2. 合并门 3: 跨文件合并为一个 `REVIEW_PACKET`, 文件顺序 = `PROMPT_FILES` 顺序 (SKILL.md 首位); 同维度字母按文件顺序重新连续编号 (id 顺移, 如 SKILL.md 有 C1、C2, 下一文件的 C1 顺移为 C3), 每条 issue 的 `location` MUST 含文件路径, 最后重算 ledger。合并后包内 id MUST 唯一。
