...
**不接**: 普通改代码 → 走常规编码流程; 代码审查 → 走 `review`; 生产部署 (不涉及 GitHub 交付) → 走 `deploy`; 正式打 tag / 发版号 / release 包装 → 走 `release`; 明说「先讨论 / 别推」时它只报告, 不动 Git。
...
- **本次更新说明**: 每次交付都整理 1-5 条本次改了什么; 同一份说明写进 commit body 和最终交付卡; 如果项目已有 changelog, 顺手补进去。
- **GitHub 交付卡**: 结论只有 `已推送完成` / `未推送: 待 GitHub 授权` / `未推送: 需先修复` / `暂不建议推送` 四种; 卡里列本次更新、环境变量字段名 + 目标位置, 真实值说「私下提供」。
