...
`profile: bounded-per-image`，不使用版本化 capability 名称。
...
plan 只输出 current-attempt `plan[.vNNN].json` 与 `shot-list[.vNNN].md`，禁止出现 current-attempt prompts/images。计划必须选择注册 style，解析品牌策略与 `gpt-image-2` 几何，支持 1-8 张图；每个锚点逐字存在于 winner 正文，excerpt 与 core meaning 分别唯一，Workflow/Checklist 默认 `icons_only`，其他可读文字受 `short_labels` allowlist 限制。`max_images=8` 是上限，不是填充配额；锚点不足、重复或正文不支持时在生成前阻断。

visual gate 在 stage 仍 running 时精确绑定五份 plan 与五份 shot-list。批准后仍停留在 visual，generate 父任务才可创建。bounded-per-image 父任务只授权最终 `bundle[.vNNN].json` 与 provider 原生 `manifest[.vNNN].md`；总控随后初始化 `generation-queue[.vNNN].json`，按 plan 顺序为每张图创建独立 child request：
...
- Set QA PASS 后父任务只按 approved anchor 顺序聚合已绑定的 child result，不按完成顺序扫描磁盘。

每图 content/style/set QA 必须 `pass`；品牌启用时 brand QA=`pass` 且 overlay=`applied`，关闭时使用对应的 `disabled-by-user|disabled-by-style-default`。生成尝试为 1-3，原生尺寸和宽高比通过 Style Spec，residual risk 必须 `none`。总控递归验收 queue、child request/result、Set QA、父 result、全部动态 prompt/image 的集合、路径和哈希，拒绝额外 current-attempt prompt/image、软链接、目录逃逸或漂移。
