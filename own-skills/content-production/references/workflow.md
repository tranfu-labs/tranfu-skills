...
2. provider 在独立任务中执行 `validate-request`，沿用自身完整的后端预检、内容分析、注册样式、品牌策略、几何、锚点和 shot-list 规则，只输出 current-attempt `plan[.vNNN].json` 与 `shot-list[.vNNN].md`，不得生成 prompt 或图片。
...
4. 批准后并行启动六个任务：五个平台分别运行 `create-illustration-request.mjs <run-dir> generate --platform <id>`；同时运行 `create-wechat-cover-request.mjs <run-dir>`。正文图 request 精确绑定 winner、标题 decision、已批准 plan 和 shot-list；封面 request 精确绑定公众号 winner、标题 decision、固定样式与 `1923x818 PNG` 约束。
5. illustration provider 每套图使用一个注册 style，逐图生成、最多三次尝试、保留通过 QA 的原生像素；品牌启用时只做确定性后叠加，品牌关闭时 `disabled-by-user|disabled-by-style-default` 是合法 QA 状态。输出 `bundle[.vNNN].json`、原生 `manifest[.vNNN].md`、prompts 和 images。公众号正文图不包含发布封面。
6. `wechat_cover` 保留独立模式的最多三次候选流程；provider 模式逐候选生成和归一化，只接受十项视觉门全部 PASS 的结果。标题结论记录为绑定最终封面哈希的 `provider_observed_exact`，并显式记录 `ocr_status=not_performed`；不得把模型视觉检查表述成确定性 OCR。独立模式的 `BEST_EFFORT` 在总控中映射为 `BLOCKED`，不得写 canonical cover。
7. 用 `set-stage.mjs` 精确绑定五个平台各自的 plan、shot-list、bundle、native manifest 20 件，再绑定当前 attempt 的封面 PNG 与 `cover.json` 2 件，共 22 件。动态 prompts/images/source/candidates 不直接进入 stage binding，由 provider result、bundle 或 cover metadata 的精确路径、哈希、QA 和集合校验递归覆盖。全部通过后进入 package。
