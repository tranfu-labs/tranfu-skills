...
2. 为五个平台分别运行 `create-illustration-request.mjs <run-dir> plan --platform <id>`。`max_images` 只从 coverage target 派生，命令行值只可作相等断言。provider 消费 `final_draft`、`title_selection`、`visual_coverage`，沿用完整后端预检、注册样式、品牌与几何规则，只输出 current plan 和 shot list。计划必须处于 minimum..target、覆盖所有 required unit、禁止重复 anchor；小红书逐页映射 4-8 张。
3. 五个 provider plan result 全部 PASS 后运行 `create-visual-decision.mjs <run-dir>`。只有该脚本生成的 current `VisualDecision` 可作为 visual gate decision；`set-gate.mjs` 在 visual 仍 running 时复算 policy、coverage、plan 和跨平台数量并精确验收十件 plan/shot-list。批准后 current stage 仍是 visual。
