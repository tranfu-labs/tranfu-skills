...
两种模式都只生成交接单，不发布。run 目录本身就是完整交付包，不再复制一份汇总目录。`handoff.md` 先展示五个平台 current 发布稿、发布图片目录、公众号封面、clean HTML 和预览，再按阶段索引当前创作简述、选题、研究 Markdown、大纲、A/B 母稿及 review、十份平台初稿/终稿及三轮 review、标题矩阵、五份 shot list、被最终采用的 prompts/native manifests，以及 QA/交接。旧版本、失败候选、临时 checkpoint 和内部控制文件不进入运营索引；Agent 最终回复也不得展示内部控制文件路径。
