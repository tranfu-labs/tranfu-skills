...
1. 把 `visual` 设为 running。为五个平台分别运行 `create-illustration-request.mjs <run-dir> plan --platform <id>`；总控从 titles gate 的 decision 文件读取 winner，不信任调用方另传正文或标题。
2. provider 在独立任务中执行 `validate-request`，沿用自身完整的后端预检、内容分析、注册样式、品牌策略、几何、锚点和 shot-list 规则，只输出 current-attempt `plan[.vNNN].json` 与 `shot-list[.vNNN].md`，不得生成 prompt 或图片。plan 支持 1-8 张；每张图的正文 excerpt 逐字存在且唯一、core meaning 唯一，Workflow/Checklist 使用 `icons_only`，其他可读文字只允许 `short_labels`。
3. 五个 provider plan result 全部 PASS 后创建版本化 visual decision。`set-gate.mjs` 在 visual 仍 running 时精确验收十件 plan/shot-list；自主模式立即批准，reviewed 模式等待确认。批准后 current stage 仍是 visual。
4. 批准后为五个平台分别运行 `create-illustration-request.mjs <run-dir> generate --platform <id>`，父 request 只授权最终 bundle/manifest；同时创建独立公众号封面 request。运行 `illustration-queue.mjs <run-dir> init` 后由 queue 派发正文 child 和封面：每套第 1 张 Canary PASS 后才派发后续图，全局最多 4 个、同套最多 2 个生成调用，封面占用同一个全局名额。
5. 每个 child 只授权自己的 prompt、candidate/source、同尺寸品牌 delivery、QA 和 result。Prompt preflight 检查文字 allowlist 与比例指令；candidate 读取真实 raster 格式、尺寸、比例和最短边，失败才创建下一 attempt，单图最多三次。限流或传输释放复用原 attempt，已通过 child 的文件和哈希保持冻结。
6. 全部 child PASS 后，每套串行执行 Set QA，检查风格、颜色、密度、重复构图和叙事顺序。失败必须返回具体 image ID 和原因，只解冻被点名图片；无法定位时整套阻断。PASS 后父任务只从已验证 child result 按 plan 顺序聚合 `bundle[.vNNN].json` 与原生 `manifest[.vNNN].md`。公众号正文图不包含发布封面。
7. `wechat_cover` 保留独立模式的最多三次候选流程；provider 模式逐候选生成和归一化，只接受十项视觉门全部 PASS 的结果。标题结论记录为绑定最终封面哈希的 `provider_observed_exact`，并显式记录 `ocr_status=not_performed`；不得把模型视觉检查表述成确定性 OCR。独立模式的 `BEST_EFFORT` 在总控中映射为 `BLOCKED`，不得写 canonical cover。
8. 用 `set-stage.mjs` 精确绑定五个平台各自的 plan、shot-list、bundle、native manifest 20 件，再绑定当前 attempt 的封面 PNG 与 `cover.json` 2 件，共 22 件。queue、child、Set QA、prompts/images/source/candidates 不直接进入 stage binding，由递归的路径、哈希、QA 和集合校验覆盖。全部通过后进入 package。

首个 visual attempt 使用无版本业务文件；重开 completed visual，或已批准计划后从 blocked 重启，递增 attempt 并使用 `.v002` controls/业务文件及 `prompts/v002`、`images/.../v002`。封面同步使用 `wechat-cover.v002.request/result.json`、`source.v002.md`、`cover.v002.png/json`、`prompts/v002/` 和 `candidates/v002/`。旧 attempt 不得覆盖或复用。重开只失效 visual 门禁、package 和 final QA，保留 titles 门禁及五个平台 winner。
