...
```text
content-production-provider: image-compression-v1
```

For that request only, MUST read `references/orchestrated-provider.md` and use
`scripts/provider-contract.mjs`. Provider mode processes exactly one bound image, writes only its
authorized staging candidate and canonical result, never uses directory mode or `--replace`, and
returns all decisions to the orchestrator. When no provider marker is present, continue with the
standalone workflow below unchanged. A partial or conflicting provider marker MUST still route to
the provider validator and return `BLOCKED`; NEVER fall back to the standalone workflow.
...
2. Choose CLI options from the user's request:
   ...
   - The explicit output path must differ from the input path; use `--replace` when replacement is authorized.
