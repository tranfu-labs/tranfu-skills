...
The favicon still shows the old logo. I want to swap the whole set — favicon, apple-touch-icon, manifest icons (plus the JSON-LD Organization logo when the page already has one) — in one coordinated push with no stragglers.
...
- **Coordinated rename checklist**: `og:image` / `og:image:secure_url` / `twitter:image` / `image_src` / every icon link / manifest icons / existing JSON-LD `logo` / CDN `Content-Type` — miss one and a stale asset lingers
