...
- **分级短路**: 门 1 → 门 2 → 门 3 (SKILL.md 先审) 逐级把关, 上一级判死就立即产出局部报告退出, NEVER 为已判死的目标继续烧后续 SubAgent——内容或容器已经不成立时, 细审措辞没有意义。注意区分: 门 2 只有在容器**真·框错** (会吸收成功标准不同的相邻任务 / 需拆分) 时才判死短路; 单纯"存在更优命名"是判断类偏好, 记为采访项并继续, NEVER 因命名偏好短路 (对齐 skill-create-workflow §4.1 "问用户、不自动切")。
...
5. T2: 门 2 任务域。仅当容器真·框错 (吸收成功标准不同的相邻任务 / 需拆分) → 短路退出 (verdict: 建议重构); 若只是存在更优命名 → 记为判断类采访项并继续。
...
- 能解析出完整 skill 目录, 但请求本身在 "整体质量提升 (本 skill)" 与 "定点改某功能 / 某具体行为、不要质量审查 (skill-create-workflow update/repair)" 之间不明确 → MUST 先一次征询让用户二选一, 再决定继续四门审查还是重定向, NEVER 默认吞成全量审查。
- 否则 (无法解析出任何目标) → 向用户要 skill 名或路径并退出。

卫语句与清单收集:

- 若 `target_dir` 不存在或其中无 `SKILL.md` → `status: needs_user_input`, 报告确切缺失路径, 退出。
- 主 agent 只做轻量收集, 按 "这文件是给 agent 读的指令, 还是给人 / 流程查的资料" 把 `target_dir` 下文件分成两桶:
  - `PROMPT_FILES` (送门 3 审): `SKILL.md` + agent 定义 (`agents/*.yaml`、`agents/*.md`) + `templates/` 下**确实是指令 / 派发模板**的文件。每个附一个 `role` 标签 (`skill-body` / `agent-def` / `subagent-template` / `instruction-fragment`), MUST 以 `SKILL.md` 排首位。
  - `NON_PROMPT_FILES` (**不送门 3**, 只留给门 4 查悬空引用): 纯数据 / 参考资料 / schema / 示例输出 (典型是 `references/` 下的表格、数据、样例文本), 以及脚本、图片等非文本资产。理由: prompt-review 的 A–G 是提示词工程清单, 拿它去量一张数据表只会产出假问题——所以按**文件角色**分, 不按 `.md` 后缀一刀切。
  - 令 `ALL_FILES` = `PROMPT_FILES` + `NON_PROMPT_FILES` (门 4 的悬空引用检查用这份全量清单)。
  - 角色拿不准的文件 → 归 `PROMPT_FILES` 并标 `role: uncertain`, 由门 3 reviewer 自行判断维度是否适用 (reviewer 模板本就允许 "映射不到维度的观察直接丢弃")。
- 读 frontmatter 拿 `name` 字段即止。NEVER 在主会话通读文件正文——正文细读属于各门 SubAgent。
...
| T3 | 门 3 reviewer-A/B/C (只审 SKILL.md) + 门 4 结构 | 3 + 1 | `SKILL.md` / `ALL_FILES` | 组内并发 | T2 pass; **SKILL.md 片段含 BLOCKER/HIGH → 短路退出** | REVIEW_PACKET 片段 + structure 结论 |
...
回收判定: 返回 `通过` → gate1: pass, 进入 T2。返回 `打回` → gate1: fail, 记录缺失字段 (这不是措辞问题, 是 "该内容不够格成为 skill" 的信号), **短路退出**; 该门的缺失字段 MUST 转成一条判断类采访项 (§7 B2), 附建议选项 [把缺的内容补上、继续留作 skill / 它不该是 skill、改成一篇普通说明文档 / 先不动它、我知道有这个隐患]。
...
回收判定 (令 `current_gap` = Top1 总分 − 当前名总分; 注意它与门 2 原生输出的 `Top1-Top2 分差` 不是同一个量):

- 当前名 == Top1 → gate2: pass, 进入 T3。
- framing 判定当前容器**真·框错** (范围边界显示它会吸收成功标准不同的相邻任务, 或建议拆分成多个容器) → gate2: fail, **短路退出** (verdict: 建议重构); 转一条判断类采访项, 附建议选项 [换一个更贴切的名字: 〈填真实候选名〉 / 拆成两个 skill、各管一件事 / 先不动它、我知道有这个隐患] (弹给用户时 `{top1}` MUST 换成真实候选名, 见 §7 词典), 说明这属重构、由 `skill-create-workflow` 执行。这是唯一会因门 2 短路的情形——改措辞救不了。
- 当前名 ≠ Top1 但同属一个成功标准/领域 (framing 未判定需拆分), 无论 `current_gap` 大小 → gate2: **pass 但 flagged**, NEVER 短路; 把命名记为一条判断类采访项 (`current_gap` ≥ 2 时标 "评分差距较大, 强烈建议复议命名"; < 2 时标 "势均力敌, 供参考"), 继续 T3, verdict 由门 3/门 4 决定。命名是用户已选定的判断, 由 B2 采访用户拍板, 用户不复议即维持现名。
...
文件角色: {ROLE}   # skill-body / agent-def / subagent-template / instruction-fragment / uncertain
按角色校准: 目标若无 description / 触发面 (纯派发模板、指令片段), A / B 维 (触发覆盖) 不适用, NEVER 硬套; 目标若不含流程, G 维 (流程完备性) 不触发。
...
```text
角色: 你是结构完整性检查员, 只读不写。
目标: {TARGET_DIR} 下的 SKILL.md; 文件清单 {ALL_FILES} (悬空引用检查需全量清单, 含数据 / 参考文件)
任务: 逐项执行本模板下方的检查表; 命中缺失即记一条; 另检查 SKILL.md 引用的文件是否
     都真实存在 (对照 {ALL_FILES}), 缺失记 HIGH (悬空引用)。
边界: 只读 {TARGET_DIR} 下文件; 不编辑; 不访问网络。
输出格式: YAML 列表, 每条含 id (S1/S2..., 片段内从 1 起编) / check / severity / location / evidence / fix。全部通过时输出空列表。
检查表:
...
1. 任一 SubAgent 无产出 / 产出不合法 (门 1 缺六字段、门 2 缺 `Top1`/分差行或评分表少于 4 行或评分表不含当前名行 (当前名总分为空)、门 3 片段非合法 YAML、门 4 非列表或列表项缺 id) → 用同一模板重派一次并附上一轮产出与格式提醒; 再失败 → 该门 (或该文件×维度组) 标 `mode: degraded`, 由主 agent 按对应清单本地补审该缺口, 并记入 `residual_risks`。位于短路判定点的门 (T1/T2/T3) MUST 先补齐结论再决定是否短路, NEVER 拿不合法产出直接放行或直接判死; 门 2 的 `current_gap` 依赖当前名总分, 校验不到当前名行时 MUST 走重派 / degraded, 不得在缺失输入上算短路。
2. 合并门 3 与门 4: 跨文件合并为一个 `REVIEW_PACKET`, 文件顺序 = `PROMPT_FILES` 顺序 (SKILL.md 首位)。**id 提升**指的是把各片段内的局部 issue id 提升为文件限定的全局 id: 各 reviewer 片段内 id 仍从 1 起编 (门 3 用维度字母+序号, 门 4 用 `S`+序号), 合并时给每条 issue 的 id 冠上来源文件前缀 (如 SKILL.md 的 `C1` → `SKILL.md#C1`, 门 4 的 `S1` → `SKILL.md#S1`, `agents/openai.yaml` 的 `C1` → `agents/openai.yaml#C1`)。文件内不重编号——重审同一文件时局部 id 稳定; 每条 issue 的 `location` 仍含 `file:line`, 最后重算 ledger。合并后包内 issue id 因带文件前缀而天然唯一, 且门 4 结构 issue 与门 3 issue 处于同一 id 空间, 可被 `issues_by_fix_kind` 与 §7 B1 以合法 id 引用。
...
SKILL_REVIEW_REPORT:
  ...
  gates:
    ...
    domain_framing:
      ...
      top1: <候选名>
      current_gap: <整数, = Top1 总分 − 当前名总分; 驱动 gate2 判定的量>
      top1_top2_gap: <整数, 门 2 原生输出的 Top1-Top2 分差; 仅供参考, 不驱动裁决>
      flagged: <true 时表示存在更优命名的判断类采访项 (非短路)>
      note: <一句话>
    prompt_quality:
      result: pass | fail | not-run
      mode: subagent | local-fallback | degraded | skipped-missing-capability | skipped-short-circuit
...
**用户文案 MUST 说人话** (适用于本阶段与门 1 / 门 2 采访的所有问题和选项): 凡是弹给用户的问题 / 选项, NEVER 直接抛内部术语; 每个选项 = 一句 "我要干什么" 的大白话 (＋可选一句括号里的为什么); 占位符 (`{top1}`、`{skill_name}`、`N`、`k`) MUST 先换成真实名字 / 具体数字再展示, 不允许把带花括号的变量原样给用户。照下表换词:

| 内部说法 | 弹给用户时说 |
|---|---|
| 补齐字段 / 缺字段 | 补上缺的内容 |
| 降级为普通文档 | 改成一篇普通说明文档 |
| 保留现状记风险 | 先不动，我知道有这个隐患 |
| 成功标准不同 | 要解决的根本不是一类问题 |
| 容器 / 容器框错 | 这个 skill 的范围划错了 |
| 排除 (某任务) | 把这个任务划到范围外 |
| 拆分 | 拆成两个 skill，各管一件事 |
| Top1 / `{top1}` / 候选名 | 直接写出真实名字 (如 daily-report) |
| current_gap / 评分差距较大 | 更好的那个名字明显更贴切 |
| 势均力敌 | 两个名字差不多，都行 |
| 确定类 / deterministic | 改法很明确，照着改就行 |
| 判断类 / judgment | 得你来拿主意 |
| BLOCKER / HIGH / MEDIUM / LOW | 致命 / 严重 / 一般 / 轻微 |
| 悬空引用 | 引用了一个不存在的文件 |
| degraded / 本地补审 | 自动审查没跑成，我手工补上 |
| 落盘 | 把改动写进文件 |

内部报告 / 日志仍可用原术语, 此表只约束**弹给用户**的文案。
...
3. 一次征询同意 (Claude Code: AskUserQuestion; Codex Plan: request_user_input; 都不可用: 交互会话则输出编号选项纯文本问题并阻塞等待; 非交互会话且无法取得用户输入 → 跳过修复阶段, 标 `remediation: skipped-no-consent-channel`, 待修项留用户手动处理, 见失败路径)。选项 = [这些都照建议改 (推荐) / 我一条条挑着改 / 这些都先不改]。选 "一条条挑" → 再对每条给 改 / 不改。
...
2. **开场 MUST 先报总量**: "有 N 处需要你拿主意, 我最多问你 N 轮, 一个一个来。" (N 展示时换成具体数字) —— 让用户一开始就知道边界, 不会觉得没完没了。
3. 逐条采访, 每条 MUST 带进度头 `[k/N]`:
   - 陈述发现 + 卡点; 给**具体选项** (≥2 个, 第一个是带一句理由的推荐默认); 需要时附一句建议。NEVER 开放式空问 (如 "你觉得呢?")。
   - 有 ≥2 个判断点且征询工具支持一次多问 (如 AskUserQuestion 单次最多 4 问) 时, MUST 合并采访而非逐条问; 每个问题的 header MUST 仍显示它在 N 里的位置。
...
5. 处置每个答复:
   ...
   - 答复为 "先不动、记风险" (用户选了 "先不动、我知道有这个隐患" 这类选项) → 记入 `REMEDIATION_SUMMARY.deferred`。
...
Workflow (走完全程 + 修复的情形; 下列每步为简洁只列 status, 实际每步 MUST 附 evidence 与 next_action, 见 §工作流的三字段格式):
1. 定位: `target_dir: own-skills/daily-report`; 分桶后 `PROMPT_FILES: [SKILL.md (role: skill-body), agents/openai.yaml (role: agent-def)]`, 无 `NON_PROMPT_FILES`; 主 agent 只读 frontmatter。status: pass。
...
5. T2: 门 2 返回 `Top1: daily-report`, 当前名 daily-report 即 Top1 (current_name_score=6, current_gap=0)。gate2: pass, 进入 T3。
...
8. T5: id 提升 (各片段带文件前缀, 门 4 的 `S1` → `SKILL.md#S1`, 无冲突); 标 fix_kind: `SKILL.md#F1` / `SKILL.md#S1` (门 4 缺 example) / `agents/openai.yaml#C1` → deterministic, `agents/openai.yaml#B1` → judgment。gate3 fail (HIGH), gate4 fail (HIGH) → `verdict: 需修改`。产出 SKILL_REVIEW_REPORT, 此刻零文件改动。
9. 修复阶段——B1: 列 3 条确定类 fix, 一次征同意, 用户选 "这些都照建议改" → 落 3 处改动, 各记 consent。B2: N=1, 开场报 "有 1 处需你拿主意"; `[1/1]` 问 "openai.yaml 里的 X 任务, 还归这个 skill 管吗? A. 划到范围外(推荐: 它和本 skill 要解决的不是一类事) B. 仍然归它管 C. 先不动、我知道有这个隐患", 用户选 A → 落盘。B3: 产出 REMEDIATION_SUMMARY (applied 4 项, interviewed 1 项, deferred []), rereview_advice: 触发面有改动, 建议重跑门 3。
</example>
