...
- framing 判定当前容器**真·框错** (范围边界显示它会吸收成功标准不同的相邻任务, 或建议拆分成多个容器) → gate2: fail, **短路退出** (verdict: 建议重构); 转一条判断类采访项, 附建议选项 [改名/拆分为 {top1} / 保留现状记风险], 说明这属重构、由 `skill-create-workflow` 执行。这是唯一会因门 2 短路的情形——改措辞救不了。
...
3. 一次征询同意 (Claude Code: AskUserQuestion; Codex Plan: request_user_input; 都不可用: 交互会话则输出编号选项纯文本问题并阻塞等待; 非交互会话且无法取得用户输入 → 跳过修复阶段, 标 `remediation: skipped-no-consent-channel`, 待修项留用户手动处理, 见失败路径)。选项 = [全部应用 (推荐) / 我逐条挑 / 全部跳过]。选 "逐条挑" → 再对每条给 应用 / 跳过。
