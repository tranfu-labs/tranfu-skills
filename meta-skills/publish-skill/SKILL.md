...
# Publish / Recommend / Add-case 到 tranfu-labs/tranfu-skills
...
- 公司仓库: `tranfu-labs/tranfu-skills` (private)
- 本地缓存: `~/.tranfu-labs/tranfu-skills/`
...
```
pattern: "<kw1>|<kw2>|<kw3>"
path: "~/.tranfu-labs/tranfu-skills"
...
2. **检测发布历史**: frontmatter 含 `published_to: tranfu-labs/tranfu-skills` → 走"更新"分支 (步骤 4 bump); 否则走"全新"分支 (步骤 3).
...
```bash
ls ~/.tranfu-labs/tranfu-skills/external-skills/<name>/cases/
```
...
```bash
test -f ~/.tranfu-labs/tranfu-skills/external-skills/<name>/cases/<recommender>.md
```
