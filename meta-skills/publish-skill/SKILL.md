---
name: publish-skill
description: 当用户说"把本地 X 发到公司库 / 这个 skill 不错分享给公司 / 给公司推荐 <url> / 给 X 加个案例"时, 完成发布 / 推荐 / 加案例流程. 在 §1 自动选 own 直路 / external in-context / external cold-start / 仅加 case 四条路径之一. 自动补全 frontmatter, 整理案例, 起草 §同类 Skill 对比 (内/外 ≤3+3 候选 + 独特价值), 引导起草 §使用技巧 (材料方案 / 推荐用法 / 已知限制 3 子段), 按 templates/ 创建 PR.
version: 0.5.0
...
## §1.6 使用技巧（路径 A/B/C 必跑, D 跳过）

目的: 帮发布者把会话里的 tacit knowledge 落到 SKILL.md, 帮阅读者**纵向上手**.
落点: SKILL.md body 新 section, 紧跟在 §同类 Skill 对比 之后. **不**进 frontmatter.

**§1.5 vs §1.6 焦点严格区分**:
- §1.5 同类对比 = **横向**: 这事还有谁在做, 我和他们有啥差别
- §1.6 使用技巧 = **纵向**: 选定这个 skill 后, 怎么用好它

两段在 SKILL.md 内并列, **互不引用, 互不重复内容**.

时机:
- 路径 A: §1.5 完成后 → 跑 §1.6 → 进步骤 5 bump
- 路径 B: §1.5 (B.4.5) 完成后 → 跑 §1.6 → 进 B.5 PR
- 路径 C: §1.5 (C.4.5) 完成后 → 跑 §1.6 → 进 C.5 PR
- 路径 D: **跳过** (case-only PR 不动 SKILL.md)
...
我会基于本会话上下文起 3-5 条引导问, 你逐条作答.
  - 单条回 'skip' → 该子段不收录
  - 回 'skip all' → 整段跳过, 写"暂无 (推荐者跳过)"占位

[回车] 开始 / [skip all] 整段跳过
```

- 用户 `skip all` → 跳到 §1.6.7, 写空占位
- 回车 / 任意非 skip 回应 → 进 §1.6.2

### §1.6.2 AI 起草引导设问 (≥3 ≤5 条, 上下文驱动)

输入:
- 本会话全部上下文 (作者/推荐者讨论过的 skill 行为 / 边界 / 失败 / 决策)
- SKILL.md `description`
- 路径 B/C 时: `source_url` 抓的官方 README

输出: 3-5 条引导问, 每条标注子段:

| 子段 | 设问示例 |
|---|---|
| **材料方案** | "你做这个 skill 时考虑过 A/B/C 哪些其他做法? 为什么选了 A?" |
| **推荐用法** | "第一次跑这个 skill 的人, 你建议先做 X 还是 Y?" |
| **已知限制** | "你之前提到 <场景>, 现在跑不动还是已经修了?" |

子段分布建议 (软约束, 不强求 3 段都覆盖): 材料方案 1-2 条 / 推荐用法 1-2 条 / 已知限制 1-2 条.

**设问质量约束**:

- ✅ 必须**具体** — 设问中名出当前 skill 的某个具体能力 / 场景 / 边界
- ❌ 禁空问 — "你有什么经验吗 / 有什么要补充的吗 / 哪些场景适合" 全删 (反过来让发布者写采访稿, 违 AI 主导心智)

### §1.6.3 用户逐条作答

AI 一次性把 3-5 条问列出, 用户可:
- 顺序作答 (短句 / 长句都行)
- 对某条回 `skip` (该子段不收录)
- 回 `skip all` (整段跳过, 跳 §1.6.7)
- 答完回 `done` 或一次性贴完所有答案
...
输入: 用户答案 (口语化) + 设问 + 子段标签
输出: §使用技巧 markdown 草稿, 3 子段结构 (见 §1.6.7 模板).

**整合约束**:

- ✅ 保留具体名词 (skill 名 / 工具名 / 场景名)
- ✅ 短句, 每子段 ≤3 bullet, 全段 ≤9 bullet
- ❌ 不加料 — 用户没说的不写, 不脑补"看起来还可能..."
- ❌ 不丢信息 — 用户提到的边界 / 痛点 / 决策都要进
- ❌ 不写空话 ("更快 / 更优雅 / 更好用" 全删)
- ❌ 不引用 §1.5 内容 (横向 / 纵向焦点严格区分)

某子段 0 条 (用户都 skip) → 该子段写一行 "暂无". 整段空 → "暂无 (推荐者跳过)" 占位.
...
### §1.6.6 字数 / bullet 上限自检

- 整段 ≤ 500 字 (中英文混算)
- 每子段 ≤ 3 bullet, 全段 ≤ 9 bullet
- 超限 → AI 主动压缩并告知用户 "这一段超长了, 我压成 X 字, 你确认吗"

### §1.6.7 写入 SKILL.md

- 路径 A: `own-skills/<name>/SKILL.md`, 位置 = §同类 Skill 对比 段之后
- 路径 B/C: `external-skills/<name>/SKILL.md` (薄指针), 位置 = §同类 Skill 对比 段之后
...
> 由 publish-skill 在发布时引导起草 (作者/推荐者答, AI 整合, 推荐者签字).
> 帮助阅读者纵向上手 — tacit knowledge 在此. 横向同类对比见上方 §同类 Skill 对比.

### 材料方案
- <1-3 行, 每行 ≤30 字; 谈作者/推荐者考虑过的其他做法 + 为什么选这个>
- (0 条则 "暂无")

### 推荐用法
- <1-3 行, 每行 ≤30 字; 谈第一次跑的建议步骤 / 前置条件 / 最佳搭配>
- (0 条则 "暂无")

### 已知限制
- <1-3 行, 每行 ≤30 字; 谈现在跑不动的场景 / 已知 bug / 设计 trade-off>
- (0 条则 "暂无")
```
...
4.5. **§同类 Skill 对比 (跑 §1.5)** — 起草 → 用户签字 → 写入 `<local>/SKILL.md` `## When to use` 段之后.

4.6. **§使用技巧 (跑 §1.6)** — 入场公告 → AI 起 3-5 条引导问 → 用户作答 → AI 整合 → 用户签字 → 写入 `<local>/SKILL.md` §同类 Skill 对比 段之后. 整段 (§1.5 + §1.6) 一并 cp 进 own-skills/<name>/ (步骤 6).
...
起草 → 用户签字 → 写入 `external-skills/<name>/SKILL.md` 薄指针正文之后. 进 B.4.6.

### B.4.6 §使用技巧 (跑 §1.6)

入场公告 → AI 起 3-5 条引导问 (基于本会话推荐者使用经验, 不是原作者经验) → 用户作答 → AI 整合 → 用户签字 → 写入 `external-skills/<name>/SKILL.md` §同类 Skill 对比 段之后. 进 B.5.
...
#       推荐场景请见 cases/ 子目录."
#    然后 append B.4.5 起草的 §同类 Skill 对比 整段, 再 append B.4.6 起草的
#    §使用技巧 整段 到 body 末尾 (见 §1.5 / §1.6 模板).
...
入场公告 → AI 起 3-5 条引导问 → 用户作答 → AI 整合 → 用户签字 → 草稿留待 C.5 写入 stub body.

cold-start 时设问要小心: 用户**没真用过**该 skill, "材料方案 / 已知限制" 子段更可能 skip. AI 设问应基于 `source_url` fetched description + 用户**推荐时表达的预期场景** (C.2 REASON + C.3 SCENARIO), 不要硬挤"你踩过哪些坑". 用户全 skip 合法, 直接写"暂无 (推荐者跳过)"占位.

### C.5 PR

同 B.5, 但 PR body `variant=external, entry=C`. dogfood log `entry=C` (`case_count` 可能 = 0). stub body 末尾 append C.4.5 起草的 §同类 Skill 对比 整段, 再 append C.4.6 起草的 §使用技巧 整段.
...
**跳过 §1.6 使用技巧** — 同上, case-only PR 不改 SKILL.md.
...
- ❌ 在 path-D (add-case) 跑 §1.6 — case-only PR 不动 SKILL.md (同 §1.5)
- ❌ §1.6 设问空泛 ("你有什么经验吗 / 有什么补充") — 必须具体到能力 / 场景 / 边界
- ❌ §1.6 整合时加料 (用户没说"更快", AI 写"比 X 更快") / 丢信息 (用户说了边界 AI 没收) / 写空话 ("更快 / 更好 / 更优雅" 全禁)
- ❌ §1.6 引用 §1.5 内容 (横向 / 纵向焦点严格区分, 两段互不引用)
- ❌ §1.6 用户全 skip 时强行编内容 — 直接写 "暂无 (推荐者跳过)" 占位
- ❌ §1.6 设问 >5 条 (发布者烦躁阈值)
- ❌ §1.6 在 frontmatter 加 `has_tips` / `tips_summary` 字段 — section in body 即可
- ❌ §1.6 二次访问追问 (一次发布期内采访一次, 不延迟跟进)
- ❌ §1.6 跨 skill 自动 backlink ("X tips 提到 Y" → Y 自动回链)
- ❌ §1.6 整段超长 (>500 字 / >9 bullet, 每子段 >3 bullet)
