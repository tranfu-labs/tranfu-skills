...
```bash
rg -l -i "<kw1>|<kw2>|<kw3>" \
  ~/.aistore-labs/claude-skills/{own,external}-skills \
  --glob "SKILL.md"
```

跳过本 skill 自己 → 读命中文件 description → 选 ≤3 个最相似. 0 命中 → 段内写"暂无".
...
再写本 skill 独特价值 ≤3 句, 每句 ≤30 字, 具体到能力 / 场景 / 输出, **禁**空话 ("更快 / 更好 / 更优雅 / 更轻量").
...
约束:
- ✅ 必须**具体** — 名出当前 skill 的能力 / 场景 / 边界
- ❌ 禁空问 — "你有什么经验 / 哪些场景适合" 全删 (反过来让发布者自写采访稿)
