...
把本地 skill 发布 / 推荐 / 加案例到公司库 `tranfu-labs/tranfu-skills`. 路径由 AI 在入口路由自动选, 不需用户指明.
...
- 在 `~/.tranfu-labs/tranfu-skills/` 创建分支 + commit + `gh pr create`.
  - own → `own-skills/<name>/`
...
- 公司仓库 push 权限 + 缓存仓库 `~/.tranfu-labs/tranfu-skills/` (由 `update-skills` 维护).
