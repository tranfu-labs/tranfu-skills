<!--
README.md 可选 section 模板.

当前 CI 不要求 README.md, 也不要求本 section. 仅在用户希望补 README 或现有 README 已经需要整理时使用.

落点建议:
- own: own-skills/{name}/README.md
- external: external-skills/{name}/README.md
不要把本段塞进 SKILL.md.
-->

## 同类 Skill 对比

> 可选说明, 帮助阅读者横向决定要装哪个 / 是否跳到更合适的同类.

### 公司库内
- [{name}](../{own|external}-skills/{name}/SKILL.md) — {对方 1 句话做什么}; **本 skill 区别**: {1 句差异}
- (≤3 条; 0 条则 "暂无")

### 外部世界
- [{name}]({url}) — {对方 1 句话做什么}; **本 skill 区别**: {1 句差异}
- (≤3 条; 0 条则 "暂无")

### 本 skill 独特价值
- {≤3 行, 每行 ≤30 字, 具体到能力 / 场景 / 输出}
