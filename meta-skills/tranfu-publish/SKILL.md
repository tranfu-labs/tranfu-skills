...
2. 识别 path + 检测多 skill
3. 起草 SKILL.md frontmatter — CI gate 只按当前 validator, 见 `references/ci-checks.md`
...
CI 当前只强制 6 字段非空 + description 长度, 详见 `references/ci-checks.md` §validate-frontmatter. 要点:

- **own**: CI 要求 `name / description / version / author / updated_at / origin` 非空; publish 约定 `origin: own`, 首发 `version: 0.1.0`, 不写 `source_url`
- **external**: CI 同样只检查 6 字段非空; publish 约定 `origin: external`, `source_url` 必填并在起草阶段 WebFetch 验 HTTP 200, `version` 上游有则填否则 fallback `1.0.0`
- **case**: 不动 frontmatter
- description 是唯一当前格式 gate: ≤ 1024 字符. 其他格式约定 (`name`=目录 / semver / 日期格式 / origin 匹配父目录 / external `source_url`) 属于 publish 阶段约定, 不写成 CI gate.
...
**body**: 按 `templates/pr-body.md` 选 variant 渲染. 自检清单区分 CI gate 与 publish 约定, AI 据实勾 (`[x]` 达成 / `[ ]` 没达成留给 reviewer 看 / `[~] N/A — 一句原因` 真不适用). 不留"需用户跑测试"项, 也不允许整段删自检清单.
