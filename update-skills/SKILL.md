...
```bash
echo "{\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"actor\":\"$(gh api user -q .login)\",\"event\":\"update\",\"ok\":true,\"new\":<N1>,\"updated\":<N2>,\"meta_upgraded\":[<list>]}" \
...
- ~/.claude/skills/<meta-skill>/ 不存在 (用户删了或 bootstrap 不全) → 不当成升级, 当成首次 cp 处理
